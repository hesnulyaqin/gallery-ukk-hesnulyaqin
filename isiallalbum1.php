...
$albumID = isset($_GET['id']) ? intval($_GET['id']) : 0; // Ganti 'album_id' menjadi 'id'
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['like_foto'])) {
    ...
    header('Location: ' . $_SERVER['PHP_SELF'] . '?id=' . $albumID); // Ganti 'album_id' menjadi 'id'
    exit;
}
?>
...
<body class="bg-gray-200">
    <a href="javascript:history.back()"
        class="inline-block px-4 py-2 bg-blue-600 text-white font-semibold rounded-md shadow hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2">
        Kembali
    </a>
    <?php
            ...
            if (isset($_SESSION['UserID'])) {
                echo "<input type='hidden' name='foto_id' value='$fotoID'>
                    <button type='submit' name='like_foto' class='like-button flex items-center'>
                        ...
                        <span class='p-2'>" . ($isLiked ? 'Unlike' : 'Like') . "</span>
